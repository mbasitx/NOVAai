...
def detect_language(text):
    ...
    if any(w in t for w in russian_latin_words):
        return "ru"

    if any(w in t for w in uzbek_words):
        return "uz"
...
async def handle_message(msg, business_id=None, source="Bot chat"):
    ...
    if not state["introduced"]:
        ...
        intro = intro_text(lang)

        # Agar birinchi xabarning o'zi "xabar yetkaz" bo'lsa, avval xabarni yetkazadi.
    ...
    if wants_to_leave_message(text):
        ...
        await send_msg(chat_id, reply, business_id)
        return
...
if __name__ == "__main__":
    uvicorn.main(app, host="0.0.0.0", port=int(os.getenv("PORT", 10000)))
